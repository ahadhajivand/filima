...
@section('content')
    ...
    <section class="section section--head section--head-fixed section--gradient section--details-bg">
        <div class="section__bg" data-bg="{{$video->image}}"></div>
        <div class="container">
            <!-- article -->
            <div class="article">
                <div class="row">
                    <div class="col-12 col-xl-8">
                        <!-- trailer -->
                        <a href="https://cdn.plyr.io/static/demo/View_From_A_Blue_Moon_Trailer-576p.mp4" class="article__trailer open-video">
                    ...
                    <div class="col-12 col-xl-8">
                        <div class="article__actions article__actions--details">
                             @if(auth()->user())
                                 @if(auth()->user()->isActive())
                                    <div class="article__download">
                                        ...
                                        دانلود :
                                        @if(!is_null($video->link_download_three))
                                            <a href="{{$video->link_download_three}}" download="#">480p</a>
                                        @endif
                                        @if(!is_null($video->link_download_two))
                                            <a href="{{$video->link_download_two}}" download="#">720p</a>
                                        @endif
                                        @if(!is_null($video->link_download_one))
                                            <a href="{{$video->link_download_one}}" download="#">1080p</a>
                                        @endif
                                    </div>
                                @else
                                ...
                                @endif
                            @else
                                <a href="{{route('auth.login')}}" class="categories__item">برای دانلود فیلم وارد سایت شوید </a>
                            @endif
                            ...
                            <form action="{{route('interest-film')}}" method="post">
                                ...
                                <input type="hidden" value="{{$video->id}}" name="interestable_id">
                    ...
                    <div class="col-12 col-xl-8">
                        ...
                        <div class="categories">
                            ...
                            @foreach($video->categories as $cate)
                                @if($cate->parent != 0)
                                    <a href="" class="categories__item">{{$cate->title}}</a>
                                @else
                                    <a href="" class="m-3">{{$video->genre_one}}</a>
                                    <a href="" class="m-3">{{$video->genre_two}}</a>
                                    <a href="" class="m-3">{{$video->genre_three}}</a>
                                @endif
                            @endforeach
                            @endif
                        </div>
                        ...
                        <div class="comments">
                            ...
                            <div class="tab-content">
                                ...
                                <div class="tab-pane fade show active" id="tab-1" role="tabpanel">
                                    ...
                                    @auth
                                    <form action="{{route('film-comment' , $video->title)}}" class="reviews__form" method="post">
                                        @csrf
    ...
    <section class="section">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <h2 class="section__title"><a href="">فیلم های  مشابه</a></h2>
                </div>
                    ...
                    <div class="section__carousel-wrap">
                        <div class="section__carousel owl-carousel" id="similar">
                            @foreach($similars as $similar)
                                @php
                                $films = $videos->where('id' ,  $similar->similar_id)->all();
                                @endphp
                            ...
                            <div class="card">
                                ...
                                <h3 class="card__title"><a href="{{route('single_film' , $film->title)}}">{{$film->title}}</a></h3>
                                <ul class="card__list">
                                    <li>رایگان</li>
