                    ...
                    <div class="row row--grid">
                        @foreach($genres as $genre)
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-2">
                                <div class="card">
                                    <a href="{{route('single_film',$genre->title)}}" class="card__cover">
                                    ...
                                    <form action="{{route('interest-film')}}" method="post">
                                        @csrf
                                        <input type="hidden" value="{{$genre->title}}" name="title">
                                        <input type="hidden" value="{{$genre->video_id}}" name="interestable_id">
                                        <input type="hidden" value="{{get_class($genre)}}" name="interestable_type">
                                    ...
                                    <h3 class="card__title"><a href="{{route('single_film',$genre->title)}}">  {{$genre->title}}</a></h3>
                                </div>
                            </div>
                        @endforeach
